...
public class Menu {
    ...
    public void stampaMenuCompleto() {
        ...
        System.out.println("");
        System.out.println(ColorEnum.PURPLE.getAnsiCode() + "Prezzo medio (calcolato su 3 portate): " + prezzoMedioMenu() + " €" + ColorEnum.RESET.getAnsiCode());


        System.out.println("\n");
        System.out.println(ColorEnum.BLUE.getAnsiCode() + "*** Antipasti ***" + ColorEnum.RESET.getAnsiCode());
        listaPortate.stream().filter(antipasto -> antipasto instanceof Antipasti).forEach(antipasto -> antipasto.printPortataColorata(ColorEnum.RESET));

        System.out.println("\n");
        System.out.println(ColorEnum.CYAN.getAnsiCode() + "*** Primi Piatti ***" + ColorEnum.RESET.getAnsiCode());
        listaPortate.stream().filter(primo -> (primo instanceof PrimiPiatti && ((PrimiPiatti) primo).getIsPiattoDelGiorno() == false)).forEach(primo -> primo.printPortataColorata(ColorEnum.RESET));

        System.out.println("\n");
        System.out.println(ColorEnum.RED.getAnsiCode() + "*** Secondi Piatti ***" + ColorEnum.RESET.getAnsiCode());
        listaPortate.stream().filter(secondo -> (secondo instanceof SecondiPiatti && ((SecondiPiatti) secondo).getIsPiattoDelGiorno() == false)).forEach(secondo -> secondo.printPortataColorata(ColorEnum.RESET));

        System.out.println("\n");
        System.out.println(ColorEnum.YELLOW.getAnsiCode() + "*** Dessert ***" + ColorEnum.RESET.getAnsiCode());
        listaPortate.stream().filter(dessert -> dessert instanceof Dessert).forEach(dessert -> dessert.printPortataColorata(ColorEnum.RESET));

        System.out.println("\n");
        System.out.println(ColorEnum.PURPLE.getAnsiCode() + "*** Bevande ***" + ColorEnum.RESET.getAnsiCode());
        listaPortate.stream().filter(bevanda -> bevanda instanceof Bevande).forEach(bevanda -> bevanda.printPortataColorata(ColorEnum.RESET));
